      ...
      >
        <div className="flex h-full flex-col bg-gray-700 text-gray-300">
          ...
          <div className="flex-grow overflow-y-auto">
            ...
            <div className="py-2 px-2 border-b border-gray-600">
              <Link
                to="/"
                className="flex w-full items-center gap-3 p-3 text-left text-gray-300 no-underline bg-gray-600 rounded hover:bg-gray-500"
                ...
                <div className="space-y-1 px-2">
                  {uploadedFiles.slice(0, 5).map((file, index) => (
                    <Link
                      key={index}
                      to={`/${file.uuid}`}
                      className={`flex w-full items-center gap-3 p-3 text-left text-gray-300 no-underline rounded hover:bg-gray-600 ${location.pathname === `/${file.uuid}` ? 'bg-gray-500' : ''
                        }`}
                      style={{ textDecoration: 'none' }}
                    >
                      <span className={`text-xl ${collapsed ? 'flex w-full justify-center' : 'flex-shrink-0'}`}>
                ...
                <div className="space-y-1 px-2">
                  {processingStages.map((stage) => {
                    ...
                    const canBeActive =
                      ...
                      Math.min(
                        processingStages.findIndex((s) => s.id === activeStep) + 1,
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 p-3 text-left text-gray-300 rounded ${isActive
                            ? 'bg-gray-500'
                            : isCompleted
                              ? 'bg-gray-600'
                              : !canBeActive
